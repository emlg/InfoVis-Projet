...
class TwoDThreeD {
  ...
  public TwoDThreeD(int width, int height) {
   ...
   double[][] solveExtrinsicMatrix(List<PVector> points2D) {
    ...
    float[][] currVector = new float[3][1];
    ...
    for(int i=0;i<4;i++){
        ...
        currVector[0][0] = points2D.get(i).x;
        currVector[1][0] = points2D.get(i).y;
        currVector[2][0] = 1;
        
        float[][] result = Mat.multiply(invK, currVector);
        
        for(int j=0; j<3 ; j++){
          projectedCorners[i][j] = result[0][j];
        }
